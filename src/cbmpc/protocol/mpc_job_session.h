...
class job_session_mp_t : public job_mp_t {
 protected:
  error_t send_impl(party_idx_t to, mem_t msg) override {
    auto network = std::static_pointer_cast<network_t>(transport_ptr);
    if (!network) return E_NET_GENERAL;
    return network->send(to, jsid, msg);
  }

  error_t receive_impl(party_idx_t from, mem_t& mem) override {
    auto network = std::static_pointer_cast<network_t>(transport_ptr);
    if (!network) return E_NET_GENERAL;
    return network->receive(from, jsid, mem);
  }

  error_t receive_many_impl(std::vector<party_idx_t> from_set, std::vector<mem_t>& outs) override {
    auto network = std::static_pointer_cast<network_t>(transport_ptr);
    if (!network) return E_NET_GENERAL;
    return network->receive_all(from_set, jsid, outs);
  }

 public:
  job_session_mp_t(party_idx_t index, std::vector<crypto::pname_t> pnames, std::shared_ptr<network_t> _network_ptr,
                   jsid_t _jsid)
      : job_mp_t(index, pnames, _network_ptr), jsid(_jsid) {}

  void set_network(party_idx_t party_idx, std::shared_ptr<network_t> ptr) { set_transport(party_idx, ptr); }

  job_session_mp_t get_parallel_job(int parallel_count, jsid_t jsid) {
    return job_session_mp_t(party_index, names, std::static_pointer_cast<network_t>(transport_ptr), jsid);
  }
...
class job_session_2p_t : public job_2p_t {
 public:
  job_session_2p_t(party_t party, crypto::pname_t pname1, crypto::pname_t pname2, std::shared_ptr<network_t> ptr,
                   jsid_t id = 0)
      : job_2p_t(party, pname1, pname2, ptr), jsid(id) {};

  void set_network(party_t party, std::shared_ptr<network_t> ptr) { set_transport(party_idx_t(party), ptr); }

  job_session_2p_t get_parallel_job(int parallel_count, jsid_t jsid) {
    return job_session_2p_t(party_t(party_index), names[0], names[1],
                            std::static_pointer_cast<network_t>(transport_ptr), jsid);
  }
