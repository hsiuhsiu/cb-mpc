...
TEST_P(ECDSAMPCTHRESHOLD, DKG) {
  ...
  // // This time we choose a different quorum
  // // TODO: this will fail since job is hardcoded to use party names 0, 1, 2
  // std::set<crypto::pname_t> quorum2;
  // quorum2.insert(all_names[1]);
  // quorum2.insert(all_names[3]);
  // quorum2.insert(all_names[4]);
