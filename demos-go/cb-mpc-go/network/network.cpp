...
JOB_SESSION_2P_PTR* new_job_session_2p(data_transport_callbacks_t* callbacks, void* go_impl_ptr, int index, char** pnames, int pname_count) {
  ...
  std::shared_ptr<parallel_data_transport_t> network =
      std::make_shared<parallel_data_transport_t>(data_transport_ptr);
  return new JOB_SESSION_2P_PTR{new job_parallel_2p_t(party_t(index), std::string(pnames[0]),
                                                     std::string(pnames[1]), network)};
}

int is_peer1(JOB_SESSION_2P_PTR* job) {
  job_parallel_2p_t* j = static_cast<job_parallel_2p_t*>(job->opaque);
  return j->is_p1();
}
...
JOB_SESSION_MP_PTR* new_job_session_mp(data_transport_callbacks_t* callbacks, void* go_impl_ptr, int party_count,
                                       int index, int parallel_id, char** pnames, int pname_count) {
  ...
  return new JOB_SESSION_MP_PTR{new job_parallel_mp_t(party_idx_t(index), pnames_vec, network,
                                                      parallel_id_t(parallel_id))};
}


int is_party(JOB_SESSION_MP_PTR* job, int party_index) {
  job_parallel_mp_t* j = static_cast<job_parallel_mp_t*>(job->opaque);
  return j->is_party_idx(party_index);
}
